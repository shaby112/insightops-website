...
export async function updateConnection(id: number, data: ConnectionUpdate): Promise<ConnectionResponse> {
...
export async function getConnectionSchema(id: number): Promise<TableSchema[]> {
  return apiFetch<TableSchema[]>(`/api/v1/connections/${id}/schema`, { auth: true });
}

export async function executeQuery(id: number, request: ExecuteRequest): Promise<ExecuteResponse> {
...
export async function getTableData(id: number, tableName: string): Promise<ExecuteResponse> {
  return apiFetch<ExecuteResponse>(`/api/v1/connections/${id}/table/${tableName}`, { auth: true });
}

export async function getQueryHistory(id: number): Promise<QueryHistoryResponse[]> {
...
export async function triggerSync(connectionId: number, incremental: boolean = true): Promise<SyncTriggerResponse> {
...
export async function getSyncStatus(connectionId: number): Promise<SyncStatusResponse> {
  return apiFetch<SyncStatusResponse>(`/api/v1/sync/${connectionId}/status`, { auth: true });
}

export async function getSyncProgress(connectionId: number): Promise<SyncProgressResponse> {
  return apiFetch<SyncProgressResponse>(`/api/v1/sync/${connectionId}/progress`, { auth: true });
}

export async function updateSyncConfig(connectionId: number, config: SyncConfigUpdate): Promise<SyncConfigResponse> {
...
export async function cancelSync(connectionId: number): Promise<{ status: string; message: string }> {
